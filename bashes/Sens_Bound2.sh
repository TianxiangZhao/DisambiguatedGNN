                        ...
                        for Negthresh in ${Negthreshs[*]}
                          do
                            for weight in ${weights[*]}
                              do
                                CUDA_VISIBLE_DEVICES=3 python deambiguous_main.py --dataset=$dataset --method=$method --Boundweight=$weight --loss=$loss --log --task=$task --epochs=$epoch --Boundrho=$Boundrho --Boundthresh=$Boundthresh --remote_K=$remote_K --aug_remote --Posthresh=$Posthresh --Negthresh=$Negthresh --seed=$seed --model=$model
