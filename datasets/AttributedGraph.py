...
class AttributedGraphDatasetNew(InMemoryDataset):
    ...
    def process(self):
        ...
        split=4
        data.cs_label = data_utils.class_size_label(data.y, split=split)
        data.group_label = data_utils.boundary_label(data.y, data.edge_index, data.cs_label, cs_group_num=split)
        data.group_label2 = data_utils.boundary_label2(data.y, data.edge_index, data.cs_label, cs_group_num=split)
        data.node_idx = torch.arange(data.x.shape[0]).long()

        print("group_label dist: {}".format(data.group_label.unique(return_counts=True)))
        print("group_label2 dist: {}".format(data.group_label2.unique(return_counts=True)))
